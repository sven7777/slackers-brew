...
import { costAllRecipes, sortRows } from "../../lib/analytics";
import { priceAsOf } from "../../lib/inventoryValue";
import { card, hdr, cell, num, th } from "../../styles";

// Analytics tab: every beer in the book and what it costs, side by side.
//
...
export default function AnalyticsTab({ recs, malts, hops, yeast, adj, settings, openRecipeCost }) {
  ...
  return (
    <div>
      ...
      <div style={card}>
        <div style={noteStyle}>
          Ingredient cost only — no packaging, labor, or utilities, and water salts are excluded
          (dosed in grams, pennies per batch). Each beer is costed against <strong>its own</strong>{" "}
          yield, so a recipe with its own average keg count is not compared on the brewery
          default. A recipe with an unpriced ingredient shows what the priced part costs, marked{" "}
          <strong>+</strong>, and is left out of the averages above — a total that omits an
          ingredient is a floor, not a cost.
